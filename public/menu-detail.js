document.addEventListener('DOMContentLoaded', function() {
    ...
    const menu = savedMenus.find((m, index) => m.category === category && index == menuIndex);
    ...
    if (menu) {
        ...
        document.getElementById('deleteMenuBtn').addEventListener('click', function() {
            if (confirm('정말로 이 메뉴를 삭제하시겠습니까?')) {
                const filteredMenus = savedMenus.filter((m, index) => !(m.category === category && index == menuIndex));
